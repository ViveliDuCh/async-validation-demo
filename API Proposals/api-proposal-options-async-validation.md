...
// Addition to existing OptionsBuilderExtensions
...
// Addition to existing OptionsBuilderDataAnnotationsExtensions
...
- `lock` + `await` is prohibited in C# — `UnnamedOptionsManager` would need a complete rewrite.
- `ConcurrentDictionary<Lazy<T>>` has no async `GetOrAdd` — the entire cache primitive would need replacement.
...
- **Binary breaking:** None. All additions are additive: new interfaces, new classes, new extension methods.
- **Source breaking:** No overload ambiguity. `ValidateDataAnnotationsAsync` and `ValidateOnStartAsync` have distinct names from their sync counterparts.
- **Behavioral:** `ValidateDataAnnotationsAsync` registers ONLY `IAsyncValidateOptions<T>`, not `IValidateOptions<T>`. This means sync `OptionsFactory.Create()` will NOT run async-only attributes. This is by design: async attributes should only run in the async pipeline. Developers using `ValidateDataAnnotationsAsync` without `ValidateOnStartAsync` will get no validation of async attributes, the API docs should make this pairing clear.
